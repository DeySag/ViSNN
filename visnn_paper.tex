...
\subsubsection{Spike Rate vs. $\lambda$ Trade-off}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.55\linewidth]{figures/spike_rate_vs_rmse.pdf}
    \caption{Spike rate vs. Val RMSE across $\lambda \in \{0.1, 0.25, 0.5, 0.75, 1.0\}$ (MTN $L=8$, 3 epochs, 200 synthetic samples). The monotonic decrease in spike rate with increasing $\lambda$ validates the threshold scaling mechanism; RMSE remains stable ($\sim$27.9--28.1 m), confirming that MTN preserves task accuracy even at lower spike rates.}
    \label{fig:spike_vs_lambda}
\end{figure}

The threshold scaling $\lambda$ provides a clean, monotonic knob: at $\lambda=0.25$, the pipeline achieves 51.0\% spike rate with 333.9 MSOP/frame (333.9 uJ/frame) — a 4.1$\times$ MAC-equivalent reduction vs. the continuous baseline (1.38 mJ/frame).
...
Both tracks show MAC-equivalent reduction at the recommended $\lambda=0.25$ MTN configuration: 4.1$\times$ for depth (3-epoch sweep) and 18.2$\times$ for detection (1-epoch functional). The 258 ms/forward CPU latency is purely Python overhead; on a digital neuromorphic core like Loihi-2, 18.5 MSOPs would execute in the low single-digit milliseconds — the figure that matters for deployment.
...
On synthetic structural analogues of KITTI and COCO, the pipeline verifies end-to-end gradient flow with Focal/SILog objectives and demonstrates a highly sparse 12.4\% spike rate on the detection track, yielding a theoretical 18.2$\times$ MAC-equivalent energy reduction. The $\lambda$ sweep on the depth track confirms a clean, monotonic trade-off between spike rate and accuracy (51.0\% spikes, 4.1$\times$ reduction at $\lambda=0.25$), validating the threshold scaling mechanism. Future work: real KITTI/COCO evaluation, end-to-end surrogate gradient fine-tuning with spike-rate regularization, and deployment on Loihi-2/SpiNNaker-2 for hardware validation.
